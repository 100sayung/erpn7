...
SELECT PLIST.P_ITCODE, PLIST.P_UNLIT, PLIST.P_AMOUNT, PCOMMOM.P_CLCODE, O_ORDER.O_CODE, O_ORDER.O_STATUS 
FROM O_PURCHASELIST PLIST JOIN O_PURCHASECOMMOM PCOMMOM 
ON PLIST.P_DOCUMENTCODE = PCOMMOM.P_DOCUMENTCODE JOIN O_ORDER 
ON O_ORDER.O_PDOCUCODE = PLIST.P_DOCUMENTCODE;--구매 VIEW

CREATE OR REPLACE VIEW REQUEST AS
SELECT IE.IE_SEQNUM, IE.IE_PRICE, IE.IE_QTY, IT.IT_PNAME, AC.CL_NAME, AC.CL_CODE, IE.IE_CPCODE,IT.IT_CODE,IE.IE_STATUS 
FROM IT JOIN S_IEPORT IE 
ON IT.IT_CODE = IE.IE_ITCODE JOIN AC_COMPANYLIST AC 
ON IE.IE_CLCODE = AC.CL_CODE;--출고-->매출 VIEW
