 ...
 * import java.util.List; import javax.servlet.http.HttpSession; import
 ...
 * com.n7.erp.bean.Category; import com.n7.erp.bean.ItemCode; import
 * com.n7.erp.service.StockMM;
 * 
 * @RestController
 * 
 * @RequestMapping(value = "/stock") public class StockRestController {
 * 
 * ModelAndView mav;
 * 
 ...
 * 
 * @RequestMapping(value = "/getcategory", method = RequestMethod.POST) public
 * ResponseEntity<List<Category>> getCategory(Category ct,HttpSession session) {
 ...
 * 
 * @RequestMapping(value = "/getitemcode", method = RequestMethod.POST) public
 * ResponseEntity<List<ItemCode>> getItemCode(ItemCode it,HttpSession session) {
 * return stmm.getItemCode(it,session); }
 * 
 * @RequestMapping(value = "/getct", method = RequestMethod.POST) public
 * ResponseEntity<String> getCt(Category ct,HttpSession session) { return
 * stmm.getCt(ct,session); }
 * 
 * @RequestMapping(value = "/itemcodeconfirm", method = RequestMethod.POST)
 * public ResponseEntity<String> itemCodeConfirm(@RequestBody
 * MultiValueMap<String, String> formData, ItemCode it,HttpSession session) { it
 * = stmm.setItemCode(formData, it); return stmm.itemCodeConfirm(it,session); }
 * 
 * @RequestMapping(value = "/modifyitemcode", method = RequestMethod.POST)
 * public ResponseEntity<String> modifyItemCode(@RequestBody
 * MultiValueMap<String, String> formData, ItemCode it,HttpSession session) { it
 * = stmm.setItemCode(formData, it); return stmm.modifyItemCode(it,session); }
 * 
 * @RequestMapping(value = "/deleteitemcode", method = RequestMethod.POST)
 * public ResponseEntity<String> deleteItemCode(@RequestBody
 * MultiValueMap<String, String> formData, ItemCode it,HttpSession session) { it
 * = stmm.setItemCode(formData, it); return stmm.deleteItemCode(it,session); }
 * 
 * @RequestMapping(value = "/getimportlist", method = RequestMethod.POST) public
 * ResponseEntity<String> getImportList(String ie_status, String date1, String
 * date2,HttpSession session) { return stmm.getImportList(ie_status, date1,
 * date2,session); }
 * 
 * @RequestMapping(value = "/confirmimportcheck", method = RequestMethod.POST)
 * public ResponseEntity<String> confirmImportCheck(@RequestBody String ipList,
 * HttpSession session) { try { ipList = URLDecoder.decode(ipList, "UTF-8");
 ...
 * public ResponseEntity<String> getByItemDealListFromItCcode(ItemCode
 * it,HttpSession session) { return stmm.getByItemDealList(it,session); }
 * 
 * @RequestMapping(value = "/getbyitemstocklist", method = RequestMethod.POST)
 * public ResponseEntity<String> getByItemStockList(ItemCode it,HttpSession
 * session) { return stmm.getByItemStockList(it,session); }
 * 
 * @RequestMapping(value = "/getmonthpayment", method = RequestMethod.POST)
 * public ResponseEntity<String> getMonthPayment(String date1, String
 * date2,HttpSession session) { return
 * stmm.getMonthPayment(date1,date2,session); } }
 */
=======
package com.n7.erp.controller;
