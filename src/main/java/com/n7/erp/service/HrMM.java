...
public class HrMM {
	...
	private String makeHRCardList(ArrayList<Member> hList) {
		...
		str.append("<tr><td>�궗吏�</td><td>�씠由�</td><td>�깮�뀈�썡�씪</td><td>�씠硫붿씪</td><td>�닔�젙�븯湲�</td></tr>");
		for (int i = 0; i < hList.size(); i++) {
			...
			str.append("<input type='button' value='�닔�젙' onclick='modifyDetail(\"" + hList.get(i).getM_id() + "\")'></td></tr>");
		}
		str.append("</table>");
		return str.toString();
	}
