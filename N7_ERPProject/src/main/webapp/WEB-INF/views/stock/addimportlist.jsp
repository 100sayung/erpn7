...
<body>
	...
	<script>
		...
		$('#addTable')
				.click(
						function() {
							var str = '<div class="div" id="div'
									+ (a++)
									+ '"><button type="button" onclick="addRow(this)">행 추가</button>	<button type="button" onclick="deleteTable(this)">테이블 삭제</button><table><caption>'
									+ clArr
									+ '</caption><tr><td>제품 코드</td><td>수량</td><td>단가</td><td>총액</td>	<td>삭제</td>	</tr>'
									+'<tr><td>'+itArr+'</td><td class="ie_qty"><input type="number" name="ie_qty" required="required"></td>'
									+'<td><input type="number" onkeyup="multiplePrice(this)" onchange="multiplePrice(this)" required="required"></td>'
									+'<td class="ie_price"><input type="number" name="ie_price" required="required"></td>'
									+'<td><input class="deleteBox" type="checkbox"></td></tr></table></div>';
							$('#contain').append(str);
						});
